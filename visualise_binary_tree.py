...
class BinaryTreeWindow: 
    ...
    def __init__(self, master: tk.Tk) -> None: 
        ...
        self.canvas = tk.Canvas(master=self.master, width=self.WIN_WIDTH, height=450)
        self.canvas.pack()
...
class BinaryTreeOnCanvas: 
    ...
    def insert(self, number: int, button: tk.Button): 
        button.destroy()
        self.root = self.__insert_aux(number, self.root, 600, 20) 

    def __insert_aux(self, number: int, current: BinaryTreeNode, x_coord: int, y_coord: int, previous_coords: tuple = None) -> BinaryTreeNode: 
        if current is None: 
            ...
            current.right_child = self.__insert_aux(number, current.right_child, x_coord + 100, y_coord + 40, (x_coord, y_coord))
        elif number < current.value: 
            current.left_child = self.__insert_aux(number, current.left_child, x_coord - 100, y_coord + 40, (x_coord, y_coord)) 
        return current
